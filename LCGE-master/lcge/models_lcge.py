...
class TKBCModel(nn.Module, ABC):
    ...
    @abstractmethod
    def add_L2(self, x: torch.Tensor):
        pass
...
class LCGE(TKBCModel):
    ...
    def forward(self, x):
        ...
        rhs = self.embeddings[0](x[:, 2])
        time = self.embeddings[2](x[:, 3])
        transt = self.embeddings[4](torch.LongTensor([0]).cuda())  # T
        ...
        return ((
               ...
               (lhs[1] * full_rel[0] + lhs[0] * full_rel[1]) @ right[1].t() +                    # L1
